...
import { ApiOwnerDto } from '../model/api.model';
import { ApiService } from '../service/api.service';
import { ToastService } from '@shared/toast/toast.service';

export interface ApiOwnerDialogData {
  apiId: string | null;
  roles: DictionaryEntryDto[];
  owner: ApiOwnerDto | null;
}

@Component({
  selector: 'app-api-owner-dialog',
  imports: [
    MatDialogModule,
    MatButtonModule,
    MatFormFieldModule,
  ...
  template: `
    <ng-container *transloco="let t; scope: 'api'; lang: lang()">
      <div class="dlg-header">
        ...
        <h2 mat-dialog-title>
          {{ data.owner ? t('api.owner.edit') : t('api.owner.add') }}
        </h2>
      </div>
        ...
        <form [formGroup]="form" class="dlg-form">
          <div class="dlg-row">
            <mat-form-field appearance="outline" class="dlg-field">
              <mat-label>{{ t('api.owner.firstName') }}</mat-label>
            ...
            <mat-form-field appearance="outline" class="dlg-field">
              <mat-label>{{ t('api.owner.lastName') }}</mat-label>
          ...
          <mat-form-field appearance="outline" class="dlg-field dlg-field--full">
            <mat-label>{{ t('api.owner.email') }}</mat-label>
            <input matInput formControlName="email" type="email" [attr.aria-required]="true" />
            @if (form.controls.email.errors?.['required']) {
              <mat-error>{{ t('common.validation.required') }}</mat-error>
            }
            @if (form.controls.email.errors?.['email']) {
              <mat-error>{{ t('api.error.invalidEmail') }}</mat-error>
            }
          </mat-form-field>

          <mat-form-field appearance="outline" class="dlg-field dlg-field--full">
            <mat-label>{{ t('api.owner.role') }}</mat-label>
          ...
          <div class="dlg-row">
            <mat-form-field appearance="outline" class="dlg-field">
              <mat-label>{{ t('api.owner.validFrom') }}</mat-label>
            ...
            <mat-form-field appearance="outline" class="dlg-field">
              <mat-label>{{ t('api.owner.validTo') }}</mat-label>
...
export class ApiOwnerDialog {
  protected readonly data = inject<ApiOwnerDialogData>(MAT_DIALOG_DATA);
  private readonly ref = inject(MatDialogRef<ApiOwnerDialog>);
  ...
  protected readonly form = this.fb.group({
    firstName: [this.data.owner?.firstName ?? '', [Validators.required, Validators.maxLength(150)]],
    lastName: [this.data.owner?.lastName ?? '', [Validators.required, Validators.maxLength(150)]],
    email: [this.data.owner?.email ?? '', [Validators.required, Validators.email, Validators.maxLength(300)]],
  ...
  protected save(): void {
    ...
    const obs = this.data.owner
      ? this.service.updateOwner(this.data.apiId, this.data.owner.id, request)
      : this.service.createOwner(this.data.apiId, request);
