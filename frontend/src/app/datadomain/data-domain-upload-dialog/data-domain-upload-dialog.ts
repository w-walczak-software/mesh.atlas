...
export interface DataDomainUploadDialogData {
  fileName: string;
}

export interface DataDomainUploadDialogResult {
  description: string | null;
}

@Component({
  selector: 'app-data-domain-upload-dialog',
  imports: [
    ReactiveFormsModule,
    MatDialogModule,
    MatButtonModule,
  ...
  template: `
    <ng-container *transloco="let t; scope: 'datadomain'; lang: lang()">
      <h2 mat-dialog-title>
        ...
        {{ t('datadomain.attachment.upload') }}
      </h2>
...
export class DataDomainUploadDialog {
  protected readonly data = inject<DataDomainUploadDialogData>(MAT_DIALOG_DATA);
  protected readonly ref = inject(MatDialogRef<DataDomainUploadDialog>);
  ...
  protected confirm(): void {
    ...
    this.ref.close({ description: desc } satisfies DataDomainUploadDialogResult);
  }
